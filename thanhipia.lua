...
local isFluentVisible = true -- Trạng thái ẩn/hiện Fluent UI
...
Toggle.Position = UDim2.new(0.05, 0, 0.05, 0) -- Di chuyển gần góc trái trên
Toggle.Size = UDim2.new(0, 40, 0, 40)

-- Bo tròn góc cho Toggle
toggle_corner.Name = "toggle_corner"
toggle_corner.CornerRadius = UDim.new(0, 10) -- Bo tròn các góc
...
-- Gọi cho cả Toggle và toggle_Image
dragify(Toggle)
dragify(toggle_Image)
...
-- Ẩn/Hiện UI khi click Toggle
toggle_Image.MouseButton1Click:Connect(function()
    isFluentVisible = not isFluentVisible
    if isFluentVisible then
        Body.Visible = true -- Hiện UI
    else
        Body.Visible = false -- Ẩn UI
    end
end)

-- Chức năng thu nhỏ/mở rộng Window (nếu bạn cần)
